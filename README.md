...
- `/`：中文首页，包含网站简介、功能亮点、按功能划分的介绍与项目开源信息。
...
首页按简介、功能、使用和开源四个栏目组织。点击导航或自然滚动时，统一的星光场和摄影背景随页面进度推进；没有自动轮播或照片风格选择器。功能区按空间漫游、时间推进、感光与大气、天体查询切换介绍，支持鼠标和键盘。穿行动画可暂停，遵循系统减少动态效果设置，页面隐藏或离开首页后停止绘制。首页摄影与穿行动效仅用于介绍，不参与观星平台物理计算。

银河照片原生宽 6000 像素，猎户座与仙女座照片原生宽 4000 像素，保留各自来源。标志采用第一款“星舟”，标签页使用独立的小尺寸图标。
