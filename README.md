...
中文星名与别名现覆盖 2,455 颗已有实测恒星。搜索“勾陈一”“北极星”“Polaris”或“HIP 11767”会定位同一对象；资料卡包含别名、目录编号、赤经赤纬、距离、星等和光谱。名称交叉匹配来源于 [Stellarium 中国星空文化](public/data/chinese-star-names-source.json)，不改变物理星表的位置或距离。

晴朗与轻雾模式支持独立的星光闪烁：靠近地平线更明显，无大气时关闭，暂停银河时间后仍可继续；详见[大气闪烁说明](public/data/stellar-scintillation.md)。13 首静谧钢琴配乐使用各区域子动机及共同主题的后段变奏，保留随机选曲和曲间留白。
...
| **星空配乐** | 13 首简洁的原创钢琴配乐，随观察位置独立随机抽取。银心温暖、外围寂冷、太阳邻域有生机。默认开启、音量 12%；曲间随机留白 25–95 秒，可关闭、静音或立即换曲。见[曲目与制作说明](public/music/README.md)。 |

首页的滚动与导航带动连续的星辰穿行动效；动画可以暂停，并遵循系统的“减少动态效果”设置。首页摄影与动效用于项目介绍。

## 开始观星

观星平台右上角可切换「English / 中文」，保留当前位置、视角与模拟时间。[直接进入英文观星平台](https://nightflight.xelope.fun/observe?lang=en)。
...
| **Gaia 实测样本** | 69,421 颗恒星 | 盖亚第三批数据发布（Gaia DR3）的明亮子样本，包含三维位置与三维速度。完整目录可查询，选中的记录按实测坐标绘制；不会把局部取样整体当作银河密度分布。见[筛选与来源](public/data/gaia-dr3-bright-6d-source.json)。 |
| **统计模型恒星** | 184,320 个固定模型源 | 按薄盘、厚盘、棒与核球、核区、恒星晕的参数化分布生成，保留位置和速度。见[模型代码](lib/rendering/model-star-catalog.ts)。 |
...
| **黑洞与引力** | 人马座 A*、天鹅座 X-1、M87* | 默认非零自旋；近景积分克尔光线轨迹，结合相对论薄盘、热辐射频移与差速流纹。黑洞位置随银河时间更新，近景镜头跟随；支持往返与曝光恢复。银河轨道使用盘、核球、暗物质晕与中心黑洞的合成引力势。见[黑洞物理模型](public/data/black-hole-physics.md)。 |
| **影像与地景** | 8 幅深空照片、2 幅 EHT 射电图、生成的标志与岩石地貌 | 照片作为影像层；地貌属于假想行星环境。见[素材声明](THIRD_PARTY_NOTICES.md)。 |
