...
export const starName=(name?:string)=>name ? commonStarNames[name]??name : '实测恒星';
...
export function drawObservedStarLabels(context:CanvasRenderingContext2D,sources:readonly ProjectedPointSource[],
  pixelRatio:number,width:number,height:number,zenith:Vector3,atmosphere:AtmospherePreset,mode:ObservationMode,sunAltitude:number):void {
  ...
  try{
    ...
    for(const {source} of visible){
      ...
      const label=starName(source.displayName);const textWidth=context.measureText(label).width;
