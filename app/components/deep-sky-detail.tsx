...
export function DeepSkyDetail({target,observer,onClose,onCentre}:{target:DeepSkyTarget;observer:Vector3;onClose:()=>void;onCentre:()=>void}){
  ...
  return <aside className="star-detail deep-sky-detail" aria-label={t("选中星云、星团或星系的资料")}>
    ...
    {target.image?<div className="object-source"><p>{t("照片署名：")}{target.image.creditShort}</p>
      <a href={target.image.sourcePage} target="_blank" rel="noreferrer">{t("查看原始观测照片与说明 ↗")}</a>
      <p>{t("照片是从太阳附近观测得到的二维图像，当前三维穿行中的内部形态仍是近似。照片中的星点不能逐个当成已载入的恒星资料。")}</p></div>:
      <p className="object-source">{t("此天体采用目录参数绘制简化轮廓，当前没有绑定观测照片。")}</p>}
    <a className="object-model-link" href="/data/SCIENCE_DISPLAY_UPDATE.md" target="_blank" rel="noreferrer">{t("查看模型与数据说明")}</a>
  </aside>;
}
