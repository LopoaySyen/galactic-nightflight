...
const steps = [
  {
    id: 'look', image: '/guide/observatory.jpg',
  ...
  {
    id: 'move', image: '/guide/position-jump.jpg',
  ...
  {
    id: 'inspect', image: '/guide/star-details.jpg',
...
export function HomeGuide({ language }: { language: HomeLanguage }) {
  ...
  return <div className="nf-guide-demo">
    ...
    <div className="nf-guide-preview">
      {steps.map((step, index) => <figure key={step.id} id={`guide-panel-${step.id}`} className={`nf-guide-shot ${index === selected ? 'is-active' : ''}`} role="tabpanel" aria-labelledby={`guide-${step.id}`} aria-hidden={index !== selected} inert={index !== selected}>
        <a className="nf-guide-image-link" href={step.image} target="_blank" rel="noreferrer" aria-label={`${step.title[language]} · ${en ? 'View full-size screenshot' : '查看完整操作截图'}`}>
          <img src={step.image} width="1363" height="936" loading="lazy" decoding="async" alt={step.alt[language]}/>
          <span className="nf-guide-expand" aria-hidden="true">{en ? 'View full size' : '查看大图'} ↗</span>
        </a>
        <figcaption><span className="nf-guide-shot-label">{en ? 'IN THE OBSERVATORY' : '观星平台实拍'}</span><span>{step.caption[language]}</span></figcaption>
